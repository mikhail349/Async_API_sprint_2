...
@pytest.mark.asyncio
async def test_get_obj_by_id(genre, api_client):
    """Проверить, что объект возвращается по запросу с корректным id."""
    data, status = await api_client.get(f"genres/{genre.id}")
    assert status == 200
...
async def test_wrong_id(genre, id_value, api_client):
    """Проверить ответ на запрос с несуществующим id."""
    data, status = await api_client.get(f"genres/{id_value}")
    assert status == 404
    assert data == {"detail": "Жанр не найден"}


@pytest.mark.asyncio
async def test_list_view(genres, api_client):
    """Проверить получение объектов списком."""
    data, status = await api_client.get("genres")
    assert status == 200
...
async def test_pagination(genres, query_params, api_client):
    """Проверить постраничный вывод объектов."""
    page_size = query_params.get("page[size]", default_values.PAGE_SIZE)
    data, status = await api_client.get("genres", **query_params)
    assert status == 200
...
@pytest.mark.asyncio
async def test_cache(genre, es, api_client):
    """Проверить что данные кешируются и возвращаются сервисом после удаления
    из elastic ."""
    await api_client.get(f"genres/{genre.id}")
